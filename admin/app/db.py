...
def delete_app_record(app_id: str) -> None:
    ...
    try:
        # First try to get the application to find the correct Application field (primary key)
        apps = get_all_apps()
        app = next((app for app in apps if app.get("id") == app_id or app.get("Application") == app_id), None)
        
        if not app:
            # If not found by id or Application, try using app_id directly as the key
            table = get_dynamodb_resource().Table(settings.APP_CONFIG_TABLE)
